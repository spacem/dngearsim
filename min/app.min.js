...
"use strict";!function(){function n(n,t){return{restrict:"A",scope:{stats:"=stats",build:"=build",altStats:"=altStats",separator:"=separator",filter:"=filter",numInSet:"=numInSet"},link:function(e,a,i){function r(){function i(n){var t=a.clone();t.css("display",u),t.html(n),l.after(t),l=t,d.push(t)}a.css("display","none");var r=e.stats;r||(r=e.altStats),angular.forEach(d,function(n,t){n.remove()});var o=!0,l=a,f={};angular.forEach(r,function(t,e){var a=n.stats[t.id];a&&a.summaryFor&&(f[a.summaryFor]=t)}),angular.forEach(r,function(a,d){var u="";if(a.id in n.stats){var l=n.stats[a.id];if(!e.filter&&"hide"in l&&l.hide)return;if(e.filter&&!l[e.filter])return;if(l.summaryFor)return;if("hideIf"in l&&l.hideIf>0){if(_.find(r,function(n){return n.id==l.hideIf}))return}if("addTo"in l&&l.addTo>0){var m=_.find(r,function(n){return n.id==l.addTo});m&&(a=_.clone(a),a.max+=m.max)}if(o||(u+=s),o=!1,"needSetNum"in a&&(a.needSetNum<=e.numInSet&&(u+="*"),u+=a.needSetNum+"&nbsp;"),u+="<strong>",e.build)if("primary"==l.element){var c=0;e.build.element&&(c=e.build.element.id),u+=t.instant(n.elements[c].name)+"&nbsp;"}else if("secondary"==l.element){var c=0;e.build.secondaryElement&&(c=e.build.secondaryElement.id),u+=t.instant(n.elements[c].name)+"&nbsp;"}if(u+=t.instant(l.name)+":</strong>&nbsp;"+l.display(a),l.combineWith>0&&angular.forEach(r,function(n,t){if(n.id==l.combineWith){if("addTo"in l&&l.addTo>0){var e=_.find(r,function(n){return n.id==l.addTo});e&&(n=_.clone(n),n.max+=e.max)}n.max!=a.max&&(u+="-"+l.display(n))}}),a.id in f){var p=f[a.id],b=n.stats[p.id];u+="&nbsp;|&nbsp;<em>"+b.display(p),u+="</em>"}}else o||(u+=s),o=!1,"needSetNum"in a&&(u+=a.needSetNum+"-Increases&nbsp;"),u+=a.id+":&nbsp;"+a.max;i(u)})}var s;s=e.separator?"&nbsp;"+e.separator+" ":"";var d=[],u=a.css("display");e.$watch("stats",function(n,t){n&&r()}),e.$watch("altStats",function(n,t){n&&r()}),e.$watch("numInSet",function(n,t){n>=0&&r()})}}}angular.module("dnsim").directive("dnsimStats",["hCodeValues","$translate",n])}();
...
"use strict";!function(){function a(a){return{getSetStats:function(t){var e=[],m={};return angular.forEach(t,function(a,t){null!==a&&null!=a.setStats&&(a.setId in m?m[a.setId].numItems++:m[a.setId]={numItems:1,stats:a.setStats})}),angular.forEach(m,function(a,t){angular.forEach(a.stats,function(t,m){t.needSetNum<=a.numItems&&e.push(t)})}),a.mergeStats(e)},getNumItemsForSet:function(a,t){var e=0;return angular.forEach(a,function(a,m){a&&a.setId&&a.setId==t&&e++}),e},getCombinedStats:function(t){var e=[];return angular.forEach(t,function(a,t){a&&(a.fullStats?e=e.concat(a.fullStats):a.stats&&(e=e.concat(a.stats)))}),a.mergeStats(e)},getCalculatedStatsFromItems:function(t,e){var m=this.getNakedStats(t),r=this.getCombinedStats(e),n=this.getSetStats(e),s=m.concat(r).concat(n);return null!=t.heroStats&&t.heroStats.length>0&&(s=s.concat(t.heroStats)),s=a.mergeStats(s),this.getCalculatedStats(t,s)},getCalculatedStats:function(t,e){function m(t){var e=a.stats[t.id];return l[e.pc]?Number(l[e.pc].max):0}function r(t){var e=a.stats[t.id];return l[e.skPc]?Number(l[e.skPc].max):0}function n(a){a.max=Math.floor(a.max*(1+m(a))*(1+r(a)))}function s(a){var t=l[a];return t?{id:a,max:Number(t.max),pc:t.pc}:{id:a,max:0,pc:0}}function i(a){a.max>0&&o.push(a)}function x(a,t,e){var m=(t+e)/2;m+=O*(z+1)*m*(1-c),m*=1+K.max;var r=m;if(W>0){var n=l[W];n&&(r=r*(1+Number(n.max))*(1-Number(u)))}if(i({id:a,max:r}),U!=W)if(U>0){var s=l[U];if(s){var x=m*(1+Number(s.max))*(1-Number(u));i({id:a+1e3,max:x})}}else i({id:a+1e3,max:m})}var o=[],l={};if(!t.conversions||!t.enemyStatCaps||!t.playerStatCaps)return[];angular.forEach(e,function(a,t){l[a.id]=a});var c;c="critResist"in t?t.critResist/100:.2;var u;u="eleResist"in t?t.eleResist/100:0;var d=s(0);n(d);var f=s(1);n(f);var g=s(2);n(g);var S=s(3);n(S);var h=s(25);h.max+=S.max*Number(t.conversions.HP),n(h);var v=s(8);v.max+=S.max*Number(t.conversions.PhysicalDefense),n(v);var p=s(1008);p.max=Math.min(.85,v.max/Number(t.enemyStatCaps.Cdefense)),i(p);var b=s(9);b.max+=g.max*Number(t.conversions.MagicDefense),n(b);var y=s(1009);y.max=Math.min(.85,b.max/Number(t.enemyStatCaps.Cdefense)),i(y);var N=s(3e3),M=s(4),C=s(5);if(!t.damageType||2!=t.damageType.id&&5!=t.damageType.id){var T=s(32),k=s(101),I=s(3001),E=s(10164);M.max+=T.max,M.max+=Math.floor(d.max*Number(t.conversions.StrengthAttack)),M.max+=Math.floor(f.max*Number(t.conversions.AgilityAttack)),M.max=Math.floor(M.max*(1+(m(M)+k.max))),M.max=Math.floor(M.max*(1+N.max+I.max)),M.max+=Math.floor(E.max*g.max),i(M),C.max+=T.max,C.max+=Math.floor(d.max*Number(t.conversions.StrengthAttack)),C.max+=Math.floor(f.max*Number(t.conversions.AgilityAttack)),C.max=Math.floor(C.max*(1+(m(C)+k.max))),C.max=Math.floor(C.max*(1+N.max+I.max)),C.max+=Math.floor(E.max*g.max),i(C)}var A=s(6),D=s(7);if(!t.damageType||1!=t.damageType.id&&4!=t.damageType.id){var P=s(33),R=s(102),V=s(3002),F=s(10165),H=s(10372);A.max+=P.max,A.max+=Math.floor(g.max*Number(t.conversions.IntelligenceAttack)),A.max=Math.floor(A.max*(1+(m(A)+R.max))),A.max=A.max*(1+N.max+V.max),A.max+=Math.floor(F.max*d.max),A.max+=Math.floor(H.max*g.max),i(A),D.max+=P.max,D.max+=g.max*Number(t.conversions.IntelligenceAttack),D.max=Math.floor(D.max*(1+(m(D)+R.max))),D.max=D.max*(1+N.max+V.max),D.max+=Math.floor(F.max*d.max),D.max+=Math.floor(H.max*g.max),i(D)}var w=s(12);w.max+=f.max*Number(t.conversions.Critical),n(w);var B=s(4012);w.max+=B.max,i(w);var L=s(1012),O=Math.min(.89,w.max/Number(t.enemyStatCaps.Ccritical)+L.max);o.push({id:1012,max:O});var j=s(103);j.max+=(d.max+g.max)*Number(t.conversions.StrengthIntelligenceToCriticalDamage),n(j),i(j);var q=s(1103),z=Math.min(1,j.max/t.playerStatCaps.CcriticalDamage+q.max);i({id:1103,max:z+2});var G=s(29);i(G);var J=Number(t.enemyStatCaps.Cfinaldamage),K=s(1029),Q=s(1030);Q.max+=K.max+Math.min(1,G.max/J),i(Q),K.max+=Math.min(Math.max(.35*Number(G.max)/J,Math.pow(Number(G.max)/J,2.2)),1),i(K);var U=0,W=0;W=t.element?a.elements[t.element.id].dmgStat:0,U=t.secondaryElement?a.elements[t.secondaryElement.id].dmgStat:0;var X=s(16);X.id!=W&&X.id!=U||i(X);var Y=s(17);Y.id!=W&&Y.id!=U||i(Y);var Z=s(18);Z.id!=W&&Z.id!=U||i(Z);var $=s(19);$.id!=W&&$.id!=U||i($),t.damageType&&1!=t.damageType.id&&0!=t.damageType.id&&4!=t.damageType.id||x(1004,M.max,C.max),t.damageType&&2!=t.damageType.id&&0!=t.damageType.id&&5!=t.damageType.id||x(1006,A.max,D.max),t.damageType&&3!=t.damageType.id||x(1001,A.max+M.max,D.max+C.max),t.damageType&&4==t.damageType.id&&i({id:1005,max:(M.max+C.max)/2}),t.damageType&&5==t.damageType.id&&i({id:1007,max:(A.max+D.max)/2});var _=s(2008);_.max=h.max/(1-p.max);var aa=s(2009);aa.max=h.max/(1-y.max);var ta=s(3008);return ta.max=(_.max+aa.max)/2,i(ta),i(d),i(f),i(g),i(S),i(h),i(v),i(b),o},getNakedStats:function(a){return a.baseStats&&a.baseStats.Strength>0?[{id:0,max:a.baseStats.Strength},{id:1,max:a.baseStats.Agility},{id:2,max:a.baseStats.Intelligence},{id:3,max:a.baseStats.Stamina}]:[]},getSkillStats:function(t,e,m){var r=null;if(angular.forEach(m,function(a,e){if(a.SkillIndex==t.id&&a.SkillLevel==t.enchantmentNum)return void(r=a)}),!r)return null;for(var n=1,s=[],i=!0;i;){var x="EffectClass"+n,o="EffectClassValue"+n;if(e&&x in e&&o in r){if(e[x]>0){var l=r[o],c=e[x],u=a.skillEffectMapping[c];if(u&&u.getVals)for(var d=u.getVals(l),f=0;f<d.length;++f)s.push(d[f]);else{var g;if(g=u&&u.mapTo?u.mapTo:1e4+c,l>0)s.push({id:g,effect:c,max:l});else if(l.toString().indexOf(";")>0){var d=l.split(";");d.length>0&&d[0]>0&&s.push({id:g,effect:c,max:d[0]})}}}}else i=!1;n++}return s},getBuildStats:function(t){var e={};return e.nakedStats=this.getNakedStats(t),e.combinedStats=this.getCombinedStats(t.items),e.setStats=this.getSetStats(t.items),e.allStats=e.nakedStats.concat(e.combinedStats).concat(e.setStats),t.heroStats&&t.heroStats.length&&(e.heroStats=t.heroStats,e.allStats=e.allStats.concat(t.heroStats)),e.allStats=a.mergeStats(e.allStats),e.calculatedStats=this.getCalculatedStats(t,e.allStats),e}}}angular.module("dnsim").factory("statHelper",["hCodeValues",a])}();
