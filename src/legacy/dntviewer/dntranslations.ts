...
export function DnTranslations() {
  ...
  this.process = function (xmlFileAsString, callback, complete) {
    console.log('processing xml translations');
    this.data = {}
    var numItems = 0;
  ...
  this.loadFromSession = function () {
    try {
      ...
      console.log('loading translations from session');
    ...
    if (this.data != null && typeof this.data === 'object') {
      ...
      xhr.onload = function (e) {
        ...
        if (this.status === 200) {
          ...
          if (fileName.toUpperCase().lastIndexOf('.JSON') === fileName.length - 5) {
            // sessionStorage.setItem('UIStrings', blobv);
            t.data = JSON.parse(blobv);
            callback('using json translations');
            complete();
          ...
          else if (fileName.toUpperCase().lastIndexOf('.LZJSON') === fileName.length - 7) {
            sessionStorage.setItem('UIStrings', blobv);
            t.loadFromSession();
